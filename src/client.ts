...
export const discordInit = async (instance: DiscordInstance): Promise<void> => {
  ...
  const getUser = async (value: string): Promise<VoiceState | null> => {
    let userValue = await instance.parseVariablesInString(value)
  ...
  const sortedTextChannelChoices = (): DropdownChoice[] => {
    ...
    instance.clientData.channels
      ...
      .forEach((channel: Channel) => {
        choices.push({ id: channel.id, label: `${instance.clientData.guildNames.get(channel.guild_id)} - ${channel.name}` })
      })
  ...
  client.on('ready', async () => {
    ...
    let newConfig = { ...instance.config, accessToken: instance.clientData.access_token }
    ...
    if (voiceChannel !== null) {
      ...
      instance.clientData.subscriptions.SPEAKING_START = await client.subscribe('SPEAKING_START', { channel_id: voiceChannel.id })
      instance.clientData.subscriptions.SPEAKING_STOP = await client.subscribe('SPEAKING_STOP', { channel_id: voiceChannel.id })
  ...
  client.on('VOICE_CHANNEL_SELECT', async (data: VoiceChannelSelectArgs) => {
    if (data.channel_id !== null) {
      ...
      instance.clientData.subscriptions.SPEAKING_START = await client.subscribe('SPEAKING_START', { channel_id: data.channel_id })
      instance.clientData.subscriptions.SPEAKING_STOP = await client.subscribe('SPEAKING_STOP', { channel_id: data.channel_id })
      instance.clientData.subscriptions.VOICE_STATE_CREATE = await client.subscribe('VOICE_STATE_CREATE', {
  ...
  client.on('VOICE_STATE_UPDATE', (voiceState: VoiceState) => {
    ...
    const index = instance.clientData.voiceChannel.voice_states.findIndex((voiceUser: VoiceState) => voiceUser.user.id === voiceState.user.id)
